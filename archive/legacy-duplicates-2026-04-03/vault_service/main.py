...
def save(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error al guardar: {str(e)}"
        ) from e
...
def get_all(username: str = Depends(verify_token), db: Session = Depends(get_db)):
    try:
        ...
        result = []
        for s in secrets:
            result.append({"site": s.site, "password": decrypt(s.encrypted_password)})
        return result
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error al recuperar: {str(e)}"
        ) from e
